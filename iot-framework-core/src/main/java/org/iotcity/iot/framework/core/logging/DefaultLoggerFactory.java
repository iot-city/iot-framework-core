...
public class DefaultLoggerFactory implements LoggerFactory {
	...
	public DefaultLoggerFactory() {
		...
		initAllLevels(this.globalConfig);
	}
	...
	public boolean config(String configFile, boolean fromPackage) {
		...
		this.globalConfig = createConfig(props, "iot.framework.core.logging.", "GLOBAL");
		if (this.globalConfig.size() == 0) initAllLevels(this.globalConfig);

		// iot.framework.core.logging=core, actor
		String loggers = props.getProperty("iot.framework.core.logging");
		if (loggers == null || loggers.length() == 0) return true;
		...
		for (int i = 0, c = keys.length; i < c; i++) {
			...
			DefaultLoggerConfig config = createConfig(props, fkey, loggerKey);
	 ...
	 */
	private static void initAllLevels(DefaultLoggerConfig config) {
		// Clear levels
		config.clearLevels();
		// Traverse all levels
		for (String level : LogLevel.ALL_LOWER_CASE) {
			...
			config.addLevel(level, tracking, tracking, getLevelDefaultColor(level, true), getLevelDefaultColor(level, false));
		}
	}
	 ...
	 */
	private static DefaultLoggerConfig createConfig(Properties props, String fkey, String defaultName) {
		...
		for (int i = 0, c = keys.length; i < c; i++) {
			...
			if ("all".equals(levelKey)) {
				...
				for (String level : LogLevel.ALL_LOWER_CASE) {
					// Get level config
					boolean classTracking = getBooleanConfig(props.getProperty(fkey + level + ".classTracking"), getTrackingDefault(level), false);
					boolean methodTracking = getBooleanConfig(props.getProperty(fkey + level + ".methodTracking"), getTrackingDefault(level), false);
					...
					int fontColor = getColorFromString(level, props.getProperty(fkey + level + ".fontColor"), true, fontHighlight);
					int bgColor = getColorFromString(level, props.getProperty(fkey + level + ".bgColor"), false, bgHighlight);
					config.addLevel(level, classTracking, methodTracking, fontColor, bgColor);
				}
				return config;
			} else {
				// Get level config
				boolean classTracking = getBooleanConfig(props.getProperty(fkey + levelKey + ".classTracking"), getTrackingDefault(levelKey), false);
				boolean methodTracking = getBooleanConfig(props.getProperty(fkey + levelKey + ".methodTracking"), getTrackingDefault(levelKey), false);
				...
				int fontColor = getColorFromString(levelKey, props.getProperty(fkey + levelKey + ".fontColor"), true, fontHighlight);
				int bgColor = getColorFromString(levelKey, props.getProperty(fkey + levelKey + ".bgColor"), false, bgHighlight);
	 ...
	 */
	private static boolean getTrackingDefault(String level) {
	 ...
	 */
	private static boolean getBooleanConfig(String value, boolean nullValue, boolean defaultValue) {
	 ...
	 */
	private static int getColorFromString(String level, String color, boolean isFontColor, boolean highLight) {
	 ...
	 */
	private static int getLevelDefaultColor(String level, boolean isFontColor) {
