...
public class LoggerConfigure extends PropertiesConfigure<LoggerConfig[]> {
	...
	public boolean config(Configurable<LoggerConfig[]> configurable, boolean reset) {
		...
		// Create list
		List<LoggerConfig> list = new ArrayList<>();
		// Create root logger configure
		LoggerConfig root = createConfig(props, "iot.framework.core.logging.", "GLOBAL");
		root.forRoot = true;
		if (root.levels == null || root.levels.length == 0) {
			root.levels = getDefaultLevels();
		}
		// Add to list
		list.add(root);
		...
		if (loggers != null && loggers.length() > 0) {
			...
			if (keys != null && keys.length > 0) {
				...
				for (int i = 0, c = keys.length; i < c; i++) {
					...
					// Create loggers configure
					LoggerConfig config = createConfig(props, fkey, loggerKey);
					// Add to list
					list.add(config);
				}
			}
		...
		// Config factory
		return configurable.config(list.toArray(new LoggerConfig[list.size()]), reset);
	}

	// --------------------------- Private static methods ----------------------------

	/**
	 * Gets all default level configures
	 * @param levelClass Level config data class
	 */
	protected final static LoggerConfigLevel[] getDefaultLevels() {
		// Create list
		List<LoggerConfigLevel> list = new ArrayList<>();
		// Traverse all levels
		for (String level : LogLevel.ALL_IN_LOWER_CASE) {
			// Add level to configure
			LoggerConfigLevel config = new LoggerConfigLevel();
			config.name = level;
		...
		// Return levels configure
		return list.toArray(new LoggerConfigLevel[list.size()]);
	}
	 ...
	 * @return Logger configure object
	 */
	protected static final LoggerConfig createConfig(Properties props, String fkey, String defaultName) {
		...
		for (int i = 0, c = keys.length; i < c; i++) {
			...
			if ("all".equals(levelKey)) {
				...
				for (String level : LogLevel.ALL_IN_LOWER_CASE) {
					// Get level config
					LoggerConfigLevel levelConfig = new LoggerConfigLevel();
				...
				// Set levels configure
				config.levels = list.toArray(new LoggerConfigLevel[list.size()]);
				return config;
			} else {
				// Get level config
				LoggerConfigLevel levelConfig = new LoggerConfigLevel();
